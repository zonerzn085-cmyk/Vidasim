...
function getCategoryForType(type: string): string {
    ...
    if (['colega de classe', 'professor', 'professora', 'diretor', 'escola', 'faculdade'].some(t => lowerType.includes(t))) {
        return 'Escola';
    }
    if (['mãe', 'pai', 'irmão', 'irmã', 'avô', 'avó', 'tio', 'tia', 'primo', 'prima', 'esposa', 'marido', 'filho', 'filha'].some(t => lowerType.includes(t))) {
        return 'Família';
    }
    if (['amigo', 'amiga', 'melhor amigo', 'namorado', 'namorada'].some(t => lowerType.includes(t))) {
        return 'Amigos';
    }
...
function Avatar({ name, base64 }: { name: string, base64?: string }) {
    if (base64) {
        return <img src={`data:image/png;base64,${base64}`} alt={name} className="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover border-2 border-gray-700 shadow-md" />;
    }
    ...
    return (
        <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br ${color} flex items-center justify-center text-white font-bold text-lg shadow-inner border-2 border-gray-800`}>
            {initials}
        </div>
    );
}
...
function RelationshipsPanel({ relationships, onClose, onInteract, onViewProfile, onOpenChat }: RelationshipsPanelProps): React.ReactElement {
  ...
  return (
    ...
    >
      <div 
        className="bg-gray-900 border border-gray-700/50 rounded-3xl shadow-2xl w-full max-w-2xl h-full sm:h-auto sm:max-h-[85vh] flex flex-col overflow-hidden relative"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-gray-800 bg-gray-900/80 backdrop-blur-md z-10 flex-shrink-0">
          <div>
              <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">Conexões</h2>
        ...
        <div className="border-b border-gray-800 bg-gray-900/50">
            <div className="flex gap-2 p-3 sm:px-6 overflow-x-auto no-scrollbar touch-pan-x">
                {categories.map((tab) => (
                    <button
                        key={tab}
                        ...
                        className={`flex-shrink-0 px-4 py-2 rounded-full text-xs font-bold transition-all border ${
                            ...
                            : 'bg-gray-800/50 text-gray-500 border-transparent hover:border-gray-600 hover:text-gray-300'
                        }`}
                    >
                        {tab}
                    </button>
        ...
        <div className="flex-grow overflow-y-auto p-4 sm:p-6 bg-gray-950 custom-scrollbar">
          <div className="grid grid-cols-1 gap-3">
            {filteredRelationships.length > 0 ? (
                filteredRelationships.map((person) => (
                <div key={person.id} className="group bg-gray-900 hover:bg-gray-800 border border-gray-800 hover:border-gray-700 p-3 sm:p-4 rounded-2xl transition-all duration-200 shadow-sm flex items-center gap-3 sm:gap-4">
                    
                    <Avatar name={person.name} base64={person.portraitBase64} />
                    
                    <div className="flex-grow min-w-0">
                        <div className="flex justify-between items-start mb-1">
                            <h3 className="font-bold text-white truncate text-sm sm:text-base">{person.name}</h3>
                        ...
                        <p className="text-[10px] sm:text-xs text-teal-500 font-bold uppercase tracking-wide truncate mb-1">{person.type}</p>
                        <RelationshipBar score={person.relationshipScore} />
                    </div>

                    <div className="flex flex-col sm:flex-row gap-2 pl-2 border-l border-gray-800">
                        ...
                        <button onClick={() => onInteract(person)} className="p-2 bg-gray-800 hover:bg-indigo-600 hover:text-white text-gray-400 rounded-xl transition-colors" title="Interagir">
                         ...
                         <button onClick={() => onViewProfile(person)} className="hidden sm:block p-2 bg-gray-800 hover:bg-gray-700 hover:text-white text-gray-400 rounded-xl transition-colors" title="Perfil">
                ...
                <div className="flex flex-col items-center justify-center py-20 text-gray-600">
                    ...
                    <p className="font-medium text-center px-4">Nenhum contato encontrado nesta categoria.</p>
                </div>
            )}
          </div>
        </div>
